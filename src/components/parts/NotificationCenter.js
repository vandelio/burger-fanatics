...
function NotificationCenter(props) {
    ...
    const [showNotificationMainCentral,setShowNotificationMainCentral] = React.useState(false);

    const Notification = (props) => {
    // {text:'Nice!', createdAt:'234324', likes:0},
    return <Box sx={{width:"100%", padding:"5px 0", display:"flex", flexDirection:"row", justifyContent:"space-between", alignItems:"center", }}>
               <Box  >
    ...
    return <>
        <div onClick={()=>{setShowNotificationMainCentral(true)}} className="notificationBadge">
        ...
        >
            ...
            <div class="notificationsArea">
                ...
                {  [{text:'Chris liked your post'},{text:'Bobby just review a new place near you'}].map((v,i)=>{
                    return <Notification key={i}  {...v} />
                }) }
                <h4>You got 1 new messages</h4>
                {  [{text:'You got a message from Burger Fanatics Introduction Team'}].map((v,i)=>{
                    return <Notification key={i} {...v} />
                })  } 
            </div>
